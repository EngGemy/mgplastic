...
.pos-banner { background:linear-gradient(135deg,#1a3a6e,#1a56db); color:white; border-radius:12px; padding:14px 20px; display:flex; gap:20px; align-items:center; margin-bottom:16px; flex-wrap:wrap; }
...
.pos-card:hover { border-color:#1a56db; transform:translateY(-2px); box-shadow:0 4px 12px rgba(26,86,219,.1); }
...
.pos-badge { position:absolute; top:6px; left:6px; background:#1a56db; color:white; font-size:10px; font-weight:700; width:22px; height:22px; border-radius:50%; display:flex; align-items:center; justify-content:center; }
...
.pos-total-pts   { font-size:1.3rem; font-weight:900; color:#1a56db; }
.pos-btn-checkout { width:100%; padding:12px; background:linear-gradient(135deg,#1a3a6e,#1a56db); color:white; border:none; border-radius:10px; font-family:'Cairo',sans-serif; font-size:14px; font-weight:700; cursor:pointer; margin-top:8px; }
...
<div class="pos-banner">
    ...
    <div class="pos-banner-stat">
        <div class="pos-banner-val">{{ $traders->count() }}</div>
        <div class="pos-banner-lbl">تاجر قطاعي</div>
    </div>
</div>
    ...
    <aside class="pos-sidebar">
        ...
        @else
            ...
            <div class="pos-cart-item">
                ...
                @else
                    <div class="pos-ci-init {{ $line['color_class'] ?? 'blue' }}">{{ $line['initials'] ?? '؟' }}</div>
                @endif
