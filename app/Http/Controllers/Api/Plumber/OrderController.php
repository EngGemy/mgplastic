...
/**
 * Plumber orders to a retail trader (channel = retail_to_plumber).
 */
class OrderController extends Controller
{
    ...
    public function store(Request $request): JsonResponse
    {
        $v = Validator::make($request->all(), [
            'retail_trader_id' => ['required', 'integer', 'exists:users,id'],
        ...
        try {
            $order = $this->orders->place(
                ...
                channel: OrderStatus::CHANNEL_RETAIL_TO_PLUMBER,
                lines: $request->input('lines'),
                meta: [
                    'note' => $request->input('note'),
                    'retail_trader_id' => (int) $request->input('retail_trader_id'),
                ],
            );

            return $this->created(
                new OrderApiResource($this->loadedOrder($order)),
                'تم إرسال الطلب للتاجر القطاعي',
            );
    ...
    {
        if (! $this->isParty($order, $request->user()) || ! $order->isPlumberChannel()) {
    ...
    {
        if (! $this->isBuyer($order, $request->user()) || ! $order->isPlumberChannel()) {
    ...
    public function cancel(Request $request, Order $order): JsonResponse
    {
        ...
        return $this->transition(
            fn () => $this->orders->cancel($order, $request->user(), $request->input('reason')),
            'تم إلغاء الطلب',
        );
    }
