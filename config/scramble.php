...
return [
    ...
    'info' => [
        ...
        'description' => <<<'MD'
# MG Plastic Mobile API

REST API for **Plumber**, **Retail Trader**, and **Wholesale Distributor** mobile apps.

## Authentication
Use `Authorization: Bearer {token}` from login or OTP verify.

## Base URL
`/api/v1/mobile/` — organized mobile endpoints  
`/api/v1/` — legacy/shared endpoints (stores, distributions, invoices)

## Quick start
1. `POST /api/v1/auth/login-plumber` — login  
2. `POST /api/v1/auth/verify-otp` — if phone not verified  
3. `GET /api/v1/mobile/me` — profile  
4. Role-specific routes under `/plumber`, `/trader`, `/distributor`

See also: `docs/API_MOBILE.md` in the repository.
MD,
    ],

    'ui' => [
        'title' => 'MG Plastic Mobile API',
    ],
